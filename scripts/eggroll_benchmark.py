...
Usage examples:
  uv run python scripts/eggroll_benchmark.py --task linear --input_dim 128 --output_dim 8
  uv run python scripts/eggroll_benchmark.py --task ill_cond --eval_budget 3200 --pop_size 16
  uv run python scripts/eggroll_benchmark.py --task step --pop_size 32 --rank 2 --sigma 0.05
"""
...
class ExperimentConfig:
    ...
    update_threshold: float = 0.0  # if >0, gate updates whose |score| < threshold
...
def make_task(cfg: ExperimentConfig) -> TaskSpec:
    ...
    if cfg.task == "linear":
        ...
        return TaskSpec(X, Y, loss_fn, True, "High-dim Linear", [(cfg.input_dim, cfg.output_dim)], "MSE regression", es_map, thread_ids)
    ...
    if cfg.task == "ill_cond":
        ...
        if mode_map == "lora":
            ...
            es_map = [0]
        thread_ids = list(range(cfg.pop_size))
        return TaskSpec(X, Y, loss_fn, True, "Ill-conditioned Linear", [(cfg.input_dim, cfg.output_dim)], "logspace feature scales (1..100)", es_map, thread_ids)
    ...
    if cfg.task == "step":
        ...
        if mode_map == "lora":
            ...
            es_map = [0]
        thread_ids = list(range(cfg.pop_size))
        return TaskSpec(X, Y, loss_fn, False, "Non-diff Step", [(cfg.input_dim, cfg.output_dim)], "sign accuracy (ES only)", es_map, thread_ids)
...
def _generate_full_noise(shapes, pop, sigma, key, antithetic, big_rand=None, start_idx=0, start_indices=None, stride=1024):
    k = key
...
def _generate_low_rank_noise(shapes, pop, sigma, rank, key, antithetic, big_rand=None, start_idx=0, stride=1024):
    k = key
    noises = [mx.zeros((pop, *shape)) for shape in shapes]
    start_indices = [start_idx + j * stride for j in range(pop)]
    for worker in range(pop):
...
def _apply_es_update(param: mx.array, perturb: mx.array, score: float, cfg: ExperimentConfig, map_class: int):
    # map_class: 0 full, 1 lora, 2 noop
    if map_class == 2:
        return param
    step = perturb
    if cfg.use_sign_updates:
        step = mx.sign(step) * score
    else:
        step = step * score
    updated = param + cfg.lr_es * step
    if cfg.int_mode:
...
def run_es(task: TaskSpec, cfg: ExperimentConfig, mode: str = "full"):
    ...
    half = pop // 2 if cfg.antithetic else pop

    # precompute big rand for deterministic slicing (keep small for local)
    big_rand = make_big_rand_matrix(max_size=10_000_000, dtype=cfg.dtype, seed=cfg.seed)
    z_gate = cfg.z_threshold
    ...
    for _ in range(cfg.steps):
        ...
        start_idx = (reuse_block * cfg.group_size * cfg.stride) % big_rand.size  # crude wraparound
        if mode == "low_rank":
            perturbations = _generate_low_rank_noise([p.shape for p in params_vals], pop, cfg.sigma, cfg.rank, k, cfg.antithetic, big_rand=big_rand, start_idx=start_idx)
        else:
            perturbations = _generate_full_noise([p.shape for p in params_vals], pop, cfg.sigma, k, cfg.antithetic, big_rand=big_rand, start_idx=start_idx)
        ...
        for idx_param, param in enumerate(params_vals):
            ...
            if cfg.antithetic:
                # use only first of each pair
                updated = param
                for idx, score in zip(pair_indices, paired_scores):
                    if cfg.update_threshold > 0 and abs(score) < cfg.update_threshold:
                        continue
                    if z_gate > 0 and abs(score) < z_gate:
                        continue
                    updated = _apply_es_update(updated, perturbations[idx_param][idx], score, cfg, map_class)
                ...
                for j in range(pop):
                    ...
                    if z_gate > 0 and abs(score) < z_gate:
                        continue
                    updated = _apply_es_update(updated, perturbations[idx_param][j], score, cfg, map_class)
                updates.append(updated)
...
def render_table(results: Dict[str, Dict], cfg: ExperimentConfig) -> str:
    ...
    for name, res in results.items():
        ...
        lines.append(f"{name}\t{final_loss:.4f}\t{res['evals']}\t{res['time']:.3f}\t{res['bandwidth_ratio']:.3f}")
    return "\n".join(lines)


def main():
    parser = argparse.ArgumentParser(description="EGGROLL vs ES vs SGD harness (MLX, script)")
    ...
    parser.add_argument("--no_use_sign_updates", action="store_false", dest="use_sign_updates")
