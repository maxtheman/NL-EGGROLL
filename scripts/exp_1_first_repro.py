...
def _generate_full_noise(
    ...
    # Process pairs if antithetic, otherwise singles
    ...
    for worker in range(0, pop, step):
        ...
        for idx_shape, shape in enumerate(shapes):
            ...
            if big_rand is not None:
                ...
                # w_start not updated here because we might need it for consistent offset across shapes?
                # Original code updated w_start, but here let's stick to simple
    ...
    for worker in range(0, pop, step):
        ...
        for idx_shape, shape in enumerate(shapes):
            ...
            if big_rand is not None:
                ...
                # Note: w_start logic for B?
                # Original code incremented w_start.
                # Here we assume big_rand is large enough and slices are distinct per worker/shape if we managed offset correctly.
                # But to match original logic, let's just grab the next slice for B from the SAME worker offset?
                # Actually original code did: w_start += total_A.
                # We should replicate that behavior if we want consistency with big_rand.

                # However, for simplicity and correctness of Antithetic:
                # We need A and B to be same for worker and worker+1.
                B_slice = slice_noise(big_rand, w_start + total_A, total_B)
            ...
            noises[idx_shape][worker] = pert
            if antithetic and (worker + 1 < pop):
                noises[idx_shape][worker + 1] = -pert
